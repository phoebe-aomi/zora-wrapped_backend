...
use zora_aomi_tools::{
    agent::{CoinMonitor, CoinWatch, MonitorConfig},
...
async fn main() -> Result<()> {
    ...
    match command {
        ...
        Command::HolderCount { address, chain } => {
            let client = load_client_from_env()?;
            let output =
                tools::get_holder_count(CoinInput { address, chain }, &client).await?;
            print_json(&output)?;
        }
        ...
        Command::Volume24h { address, chain } => {
            let client = load_client_from_env()?;
            let output =
                tools::get_24h_volume(CoinInput { address, chain }, &client).await?;
            print_json(&output)?;
        }
...
fn load_monitor_config(path: &str) -> Result<Vec<CoinWatch>> {
    ...
    let watches: Vec<RawWatch> = serde_json::from_str(&raw)
    ...
    Ok(watches
        ...
        .map(|w| CoinWatch {
            name:                   w.name,
            address:                w.address,
            chain:                  w.chain.unwrap_or_else(zora_aomi_tools::types::default_chain),
